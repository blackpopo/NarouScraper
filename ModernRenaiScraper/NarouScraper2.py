...
def get_text(url):
    try:
        ...
        def check_conversation(line):
            if line.lstrip('「').lstrip('『').rstrip('」').rstrip('』') == '':
                return False
...
def get_page(url, ncodes):
    ...
    if indices == None:
        ...
        lines = get_text(url)
        if lines != []:
            with open(os.path.join(r"C:\Users\Atsuya\PycharmProjects\NarouScraping\ModernRenai", ncode + '.txt'), 'a', encoding='utf-8') as f:
                f.write('\n'.join(lines))
        else:
